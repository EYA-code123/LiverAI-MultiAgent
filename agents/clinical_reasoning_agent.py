...
class ClinicalReasoningAgent:
    ...
    def predict(self, agent_results):
        ...
        fatty_prediction = fatty.get("prediction")
        fibrosis_prediction = fibrosis.get("prediction")
        cirrhosis_prediction = cirrhosis.get("prediction")

        fatty_probability = fatty.get("probability")
        fibrosis_probability = fibrosis.get("probability")
        cirrhosis_probability = cirrhosis.get("probability")
        ...
        if fatty_prediction is not None:
            findings.append(
                f"Fatty liver prediction: {fatty_prediction}"
            )

        if fibrosis_prediction is not None:
            findings.append(
                f"Fibrosis prediction: {fibrosis_prediction}"
            )

        if cirrhosis_prediction is not None:
            findings.append(
        ...
        return {
            ...
            "findings": findings,
            "fatty_liver_probability": fatty_probability,
            "fibrosis_probability": fibrosis_probability,
            "cirrhosis_probability": cirrhosis_probability,
            "status": "completed"
        }
